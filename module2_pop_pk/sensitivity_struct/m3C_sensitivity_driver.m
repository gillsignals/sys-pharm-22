...
p0.V3 = 40 * V3ext; % L
CL = .100; % L hr-1
p0.kc1 = CL/p0.V1; % hr-1

p0.k12 = 0.1 * p0.V2/p0.V1;  % hr-1
p0.k21 = 0.1; % hr-1
p0.k13 = 0.5 * p0.V3/p0.V1;  % hr-1
p0.k31 = 0.5 ; % hr-1

koff = 0.1; %hr-1
p0.kDBon = koff/6.5; % nM-1 hr-1   % Kd = 6.5 = koff/kon
p0.kDBoff = koff;  %hr-1
...
for i=1:length(p0names)
    ...
    t = [t t1];
    y = [y y1];
    Sens(i) = ((auc(i)-auc0)/auc0) / ((p.(p0names{i}) - p0.(p0names{i})) / p0.(p0names{i})); % relative sensitivity vs parameter
    SensB(i) = ((auc(i)-auc0))/((p.(p0names{i})-p0.(p0names{i}))); % absolute sensitivity vs parameter
    SensAbs(i) = ((auc(i)-auc0)/auc0); % relative change (not normalized to parameter)
    SensT = ((y1-y0)./y0)/((p.(p0names{i})-p0.(p0names{i}))/p0.(p0names{i})); % time-dependent relative sensitivity
...
xlabel('Time (hr)')
    ...
    xlabel('Parameters')
    xticks([1:1:length(p0names)])
    xticklabels(p0labels);
    xlim([.5 (length(p0names)+.5)])
...
for j=1:length(p0names)
	...
	Sens2(i,j) = ((auc2(i,j)-auc(j))/auc(j)) / ((p.(p0names{i}) - p0.(p0names{i})) / p0.(p0names{i})); % relative sensitivity vs parameter
    Sens2Abs(i,j) = ((auc2(i,j)-auc0)/auc0);

    y0=y(:,j+1);
    Sens2T = ((y1-y0)./y0)/((p.(p0names{i})-p0.(p0names{i}))/p0.(p0names{i})); % time-dependent relative sensitivity
...
for j=1:length(p0names)
    ...
    Sens3(i,j) = ((auc3(i,j)-auc0)/auc0) / ((p.(p0names{j}) - p1.(p0names{j})) / p1.(p0names{j})); % relative sensitivity vs parameter
    Sens3T = ((y1-y0)./y0)/((p.(p0names{j})-p1.(p0names{j}))/p1.(p0names{j})); % time-dependent relative sensitivity
...
for j=1:length(p0names)
    p=p1;
	p.(p0names{j})=p1.(p0names{j})*(1.0+ParamDelta);    
    [auc3(i,j),t1,y1] = m3C_sim(p,OutputVar,TimeLen,'off');
    Sens3(i,j) = ((auc3(i,j)-auc0)/auc0) / ((p.(p0names{j}) - p1.(p0names{j})) / p1.(p0names{j})); % relative sensitivity vs parameter
    Sens3T = ((y1-y0)./y0)/((p.(p0names{j})-p1.(p0names{j}))/p1.(p0names{j})); % time-dependent relative sensitivity
    [maxS,indT] = max(abs(Sens3T));
