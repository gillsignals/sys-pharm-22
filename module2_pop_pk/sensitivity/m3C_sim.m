...
y0 = [D0/V1 0 0 0 B0 0]'; % nM - Initial conditions vector
% 1 = drug in blood
% 2 = drug in tumor 1
% 3 = drug in body
% 4 = drug in degr
...
[T,Y] = ode45(@m3C_eqns,[0:TimeLen/1000:TimeLen],y0,options,p);

DrugInBolus = ones(length(T),1)*(D0/V1) ;

TotalFreeD(:,1) = Y(:,1)*V1;
TotalFreeD(:,2) = Y(:,2)*V2;
TotalFreeD(:,3) = Y(:,3)*V3;
TotalBoundD(:,1) = Y(:,6)*V1;

%% MASS BALANCE

DrugIn = q*T + DrugInBolus*V1 ; % cumulative drug into system
...
ax2=subplot(4,2,2);
plot(ax2,T,Y(:,1)*V1+Y(:,6)*V1,'k','linewidth',3)
...
ax4=subplot(4,2,4);
plot(ax4,T,Y(:,2)*V2,'k','linewidth',3)
...
ax8=subplot(4,2,6);
plot(ax8,T,Y(:,3)*V3,'k','linewidth',3)
